...
def get_events_before_5_mins(filename, airball_list, game_ids, play_by_play_directory):
    list_of_lists = []
    count = 0
        ...
        for arr in event_ids:
            ...
            try:
                ...
                if end_time < 7:
                    ...
                    final_time = '00:0' + str(begin_time) + ':' + edited_time[3:]
                    #print('begin time ',final_time)
                    #print(final_time, time_remaining)
                    list_of_plays = get_plays_within_times(play_by_play_directory + '/' + filename, arr[1], final_time, time_remaining, period, [])
                    list_of_lists.append(list_of_plays)
                    #print(list_of_plays)
                    count += 1
                else:
                    #print('12:00:00', time_remaining)
                    list_of_plays = get_plays_within_times(play_by_play_directory + '/' + filename, arr[1], '00:12:00', time_remaining, period, [])
                    list_of_lists.append(list_of_plays)
                    #print(list_of_plays)
                    count += 1
            except:
...
def output_list_for_5_mins_before_airball(filename):

    list = get_events_before_5_mins(filename,airball_list, game_ids, play_by_play_directory)
